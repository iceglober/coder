...
- ✅ **Full eval grid complete: 15/15 tasks pass — 5 types × 3 toolchains** (pnpm-vitest · pytest · go). Same difficulty profile across all three (Advanced ~110–140s, cross-module).
- ✅ **Ambiguous-hard tier (the real staff-level signal) + LLM judge**: a 6th task type — `{js,py,go}-architect` — an UNDER-SPECIFIED brief (design an extensible pricing pipeline; no test spec). Graded by an **LLM judge** (`coder-server/src/eval/judge.ts`, reused by `run.ts` via a `judge` rubric field) that scores coder's diff + report as a strict staff reviewer, plus a `verify` gate. The judge **discriminates** — it passed real designs (Protocol/interface-based pipelines) and failed non-attempts with accurate grounded reasoning, so it's not rubber-stamping.
- ✅ **Eval finding #2 (caught + fixed)**: coder **over-asked on explicitly-delegated open briefs** — on "decide the design yourself", it called `ask_user` and wrote zero code (js decided + passed; py/go asked + failed — same prompt, inconsistent). **Fix:** charter + INVESTIGATOR now treat an explicit delegation ("decide yourself / your call / no spec — design it") as a *mandate to design, not a prompt to ask*. After the fix all 3 architect tasks decide + implement + pass. (`prompt.ts`.)
