...
fn instructions() -> String {
    enclose(
        ...
        ASSESS — prove it's done with HARD EVIDENCE, for both you and the user. Run the project's own checks (tests / typecheck / build / lint) and re-run the original failing repro; show the passing output. Match the evidence to the KIND of work: backend/library → the test suite; a script → run it and check its output; FRONTEND/UI → you cannot see the page, so start the dev server (`job_start`) and use the `web_check` tool (or the project's e2e/Playwright suite) to confirm it renders with no console errors, failed requests, or uncaught exceptions and shows what it should. Never claim done without evidence that fits the work.\n\n\
